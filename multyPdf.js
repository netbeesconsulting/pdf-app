...
const {pdfZipDownload} = require('./pdfZip');
...
async function downLoadndMultyEncryptPdf(pdfUrl, user_email, product_id, licensed_number, customer_name, purchase_date, req, res) {
    var file_name2 = `Order ${licensed_number} licensed to ${customer_name} on ${purchase_date}`;
    var file_name3 = `Order_${licensed_number}_licensed_to_${customer_name}_on_${purchase_date}`;
    var FolderName = `${customer_name}_on_${purchase_date}`;

    const urlArray = pdfUrl.split(',').map(url => url.trim());

    if (urlArray.length === 1) {
        return res.status(400).json({ error: 'one Url sendded' });
    } else {
        ...
        const downloadPromises = urlArray.map(async (url, index) => {
            axios({
                ...
                .then(response => {
                    ...
                    const downloadedFileName = `Order_${licensed_number}_licensed_to_${customer_name}_on_${purchase_date}-${index + 1}`;
                                                ...
                                                setTimeout(() => {
                                                    ...
                                                    axios(config)
                                                        .then(async function (response) {
                                                            console.log(JSON.stringify(response.data.outputUrl));

                                                            let NewUrlPdf = response.data.outputUrl

                                                            pdfUrls.push({ url: NewUrlPdf });
                                                            pdfZipDownload(NewUrlPdf, pdfUrls, urlArray,req, res,FolderName,file_name3)
...
module.exports = { downLoadndMultyEncryptPdf }